        ...
        for keys in feat_map_keys:
            ...
            for j in range(0, M):
                ...
                if j==0:
                    ...
                    combined_features=torch.cat((combined_features, combined), dim=0)
            if cnter == 0:
                Combined = 1.0 * combined_features
            else:
                if Combined.shape[2] != combined_features.shape[2] or Combined.shape[3] != combined_features.shape[3]:
                    combined_features = F.interpolate(combined_features, size=(Combined.shape[2], Combined.shape[3]), mode='bilinear')
                Combined = torch.cat((Combined, combined_features), dim=1)
            cnter += 1
        if ix == 0:
